...
main() {
    int    i, j, k, p, np, op, ranpat[NUMPAT+1], epoch, lb_hidden, ub_hidden;
    ...
    for( epoch = 0 ; epoch < 100000 ; epoch++) {    /* iterate weight updates */
        ...
        for( np = 1 ; np <= NumPattern ; np++ ) {    /* repeat for all the training patterns */
            p = ranpat[np];
	    for( k = 1 ; k <= PART ; k++ ){
		    for( j = lb_hidden ; j <= ub_hidden ; j++ ) {    /* compute hidden unit activations */
	...
	/*              0DeltaO[k] = Target[p][k] - Output[p][k];     Sigmoidal Outputs, Cross-Entropy Error */
	/*              DeltaO[k] = Target[p][k] - Output[p][k];     Linear Outputs, SSE */
		    }
		    lb_hidden = ub_hidden + 1;
		    ub_hidden = (NUMHID/PART)*k;
	    }
	    lb_hidden = 1;
	    ub_hidden = (NUMHID/PART);
	    for( k = 1 ; k <= PART ; k++ ){
		    for( j = lb_hidden ; j <= ub_hidden ; j++ ) {    /* 'back-propagate' errors to hidden layer */
		        SumDOW[j] = 0.0 ;
		    ...
		    for( j = lb_hidden ; j <= ub_hidden ; j++ ) {     /* update weights WeightIH */
